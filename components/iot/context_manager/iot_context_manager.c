 ...
 *          can be disabled by setting the IOT_CONTEXT_MANAGER_DISABLE_LOGS to 1.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having IOT_CONTEXT_MANAGER_DISABLE_LOGS has no effect.
 * @{
 */
#if (IOT_CONTEXT_MANAGER_DISABLE_LOGS == 0)
...
/**@brief Looks up context table for specific IPv6 address. */
                                    ...
                                    iot_context_t         * p_context)
{
    ...
    if (table_id != IOT_CONTEXT_MANAGER_MAX_TABLES)
    {
          ...
          if(err_code == NRF_SUCCESS)
          {
               ...
               memcpy(p_internal_context->prefix.u8, p_context->prefix.u8, IPV6_ADDR_SIZE);
           }
           else
           {

 ...
 *          can be disabled by defining the UDP_DIABLE_LOGS.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having UDP6_DISABLE_LOGS has no effect.
 * @{
 */
#if (UDP6_DISABLE_LOGS == 0)
...
static socket_t  m_socket[UDP6_MAX_SOCKET_COUNT];                                        /**< Table of sockets managed by the module. */


/** @brief Initializes socket managed by the module. */
static void socket_init(socket_t * p_socket)
{
...
uint32_t udp6_socket_free(const udp6_socket_t * p_socket)
{
    ...
    socket_init(&m_socket[p_socket->socket_id]);
                          ...
                          iot_pbuffer_t       * p_packet)
{
    ...
    uint32_t err_code;
    const socket_t  * p_skt       = &m_socket[p_socket->socket_id];
    ...
    if((p_packet->length > UDP_HEADER_SIZE) && (p_ip_header->length > UDP_HEADER_SIZE))
    {
        ...
        if (index < UDP6_MAX_SOCKET_COUNT)
        {
            ...
            if (m_socket[index].rx_cb != NULL)
            {
                ...
                UNUSED_VARIABLE(m_socket[index].rx_cb(&sock, p_ip_header, p_udp_header, process_result, p_packet));

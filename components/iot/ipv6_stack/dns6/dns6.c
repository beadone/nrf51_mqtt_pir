 ...
 *          can be disabled by defining the DNS6_DISABLE_LOGS.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having DNS6_DISABLE_LOGS has no effect.
 * @{
 */

#if (DNS6_DISABLE_LOGS == 0)
...
#define VERIFY_MODULE_IS_INITIALIZED()
#define NULL_PARAM_CHECK(PARAM)
#defube EMPTY_PARAM_CHECK(PARAM)
...
static void query_init(uint32_t index)
{
    ...
    {
        UNUSED_VARIABLE(nrf51_sdk_mem_free(m_pending_queries[index].p_hostname));
    }
...
static uint32_t query_send(uint32_t index)
{
    ...
    if (err_code == NRF_SUCCESS)
    {
        dns_question_footer_t question_footer;
        ...
        // Add question footer.
        question_footer.qtype  = HTONS(DNS_QTYPE_AAAA);
        question_footer.qclass = HTONS(DNS_QCLASS_IN);
...
uint32_t dns6_query(const char * p_hostname, dns6_evt_handler_t evt_handler)
{
    ...
    err_code = nrf51_sdk_mem_alloc(&p_hostname_buff, &hostname_length);
    ...
    if (err_code == NRF_SUCCESS)
    {
        ...
        if (err_code != NRF_SUCCESS)
        {
            UNUSED_VARIABLE(nrf51_sdk_mem_free(p_hostname_buff));
        }
    }
    else
    {

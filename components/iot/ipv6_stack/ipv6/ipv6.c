 ...
 *          can be disabled by defining the IPV6_DISABLE_LOGS.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having IPV6_DISABLE_LOGS has no effect.
 * @{
 */

#if (IPV6_DISABLE_LOGS == 0)
...
static uint32_t ipv6_input(iot_interface_t * p_interface, iot_pbuffer_t * p_pbuffer)
{
    ...
    // For now any packet buffer is freed after processing.
                                    ...
                                    ble_6lowpan_event_t * p_6lo_event)
{
    ...
    if(rx_failure == true)
    {
        UNUSED_VARIABLE(nrf51_sdk_mem_free(p_6lo_event->event_param.rx_event_param.p_packet));
    }
...
uint32_t ipv6_init(const ipv6_init_t * p_init)
{
    ...
    UNUSED_VARIABLE(nrf51_sdk_mem_init());

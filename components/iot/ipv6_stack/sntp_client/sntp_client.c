...
/**
 * @defgroup iot_sdk_sntp Module's Log Macros
 ...
 *          can be disabled by defining the SNTP_DISABLE_LOGS.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having SNTP_DISABLE_LOGS has no effect.
 * @{
 */

#if (SNTP_CLIENT_DISABLE_LOGS == 0)

#define SNTP_TRC  app_trace_log                                           /**< Used for logging details. */
#define SNTP_ERR  app_trace_log                                           /**< Used for logging errors in the module. */
#define SNTP_DUMP app_trace_dump                                          /**< Used for dumping octet information to get details of bond information etc. */

#else  // SNTP_DISABLE_LOGS

#define SNTP_TRC(...)                                                     /**< Diasbles detailed logs. */
#define SNTP_ERR(...)                                                     /**< Diasbles error logs. */
#define SNTP_DUMP(...)                                                    /**< Diasbles dumping of octet streams. */

#endif  // SNTP_DISABLE_LOGS
/** @} */
                                   ...
                                   iot_pbuffer_t       * p_rx_packet)
{
    ...
    SNTP_TRC("[SNTP]: >> ntp_server_response\r\n");
    ...
    {
        SNTP_ERR("[SNTP]: Unexpected NTP response received.\r\n");
    ...
    else
    {
        ...
        {
            SNTP_ERR("[SNTP]: Received erroneous NTP response.\r\n");
        ...
        else
        {
            ...
            {
                SNTP_ERR("[SNTP]: Received bad NTP response.\r\n");
            ...
            else
            {
                ...
                {
                    SNTP_TRC("[SNTP]: Received Kiss-o'-Death packet.\r\n");
...
uint32_t sntp_client_init(const sntp_client_init_param_t * p_sntp_client_init_param)
{
    ...
    SNTP_TRC("[SNTP]: >> sntp_client_initialize\r\n");
...
uint32_t sntp_client_local_time_get(time_t * p_current_time)
{
    ...
    SNTP_TRC("[SNTP]: >> sntp_client_local_time_get\r\n");
...
static uint32_t sntp_query_send()
{
    ...
    if (err_code != NRF_SUCCESS)
    {
        SNTP_TRC("[SNTP]: An error occured while getting local time value. \r\n");
        return err_code;
    }
    ...
    if (err_code == NRF_SUCCESS)
    {
        ...
        if (err_code != NRF_SUCCESS)
        {
            SNTP_ERR("[SNTP]: Unable to send query on UDP socket. Reason %08lx.\r\n", err_code);
    ...
    {
        SNTP_ERR("[SNTP]: No memory to allocate packet buffer.\r\n");
    }
                                  ...
                                  bool          sync_local_time)
{
    ...
    uint32_t err_code = NRF_SUCCESS;
    SNTP_TRC("[SNTP]: >> sntp_client_server_query\r\n");
    ...
    if (m_sntp_client_state == SNTP_CLIENT_STATE_BUSY)
    {
        if (is_it_time_to_retransmit())
        {
            ...
            if (m_retransmission_count > SNTP_MAX_RETRANSMISSION_COUNT)
            {
                ...
                SNTP_TRC("[SNTP]: NTP server did not respond to query.\r\n");
                return;
            }
            else
            {
                SNTP_TRC("[SNTP]: Query retransmission [%d].\r\n", m_retransmission_count);
...
uint32_t sntp_client_uninitialize()
{
    ...
    SNTP_TRC("[SNTP]: >> sntp_client_uninitialize\r\n");

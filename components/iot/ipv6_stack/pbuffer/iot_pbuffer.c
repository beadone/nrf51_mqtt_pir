 ...
 *          can be disabled by defining the IOT_PBUFFER_DIABLE_LOGS.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having IOT_PBUFFER_DISABLE_LOGS has no effect.
 * @{
 */
#if (IOT_PBUFFER_DISABLE_LOGS == 0)
...
#endif // IOT_PBUFFER_DISABLE_LOGS
...
static uint32_t pbuffer_allocate(pbuffer_t ** pp_buffer, uint32_t length, iot_pbuffer_flags_t flags)
{
    ...
    for (index = 0; index < IOT_PBUFFER_MAX_COUNT; index ++)
    {
        if (m_pbuffer[index].allocated_length == 0)
        {
            ...
            {
                err_code = nrf51_sdk_mem_alloc(&m_pbuffer[index].buffer.p_memory, &m_pbuffer[index].allocated_length);
                                ...
                                iot_pbuffer_t              * p_pbuffer)
{
    ...
    if (err_code == NRF_SUCCESS)
    {
        ...
        if (realloc_len > p_alloc_buffer->allocated_length)
        {
            ...
            if (p_param->flags != PBUFFER_FLAG_NO_MEM_ALLOCATION)
            {
                err_code = nrf51_sdk_mem_alloc(&p_new_mem, &realloc_len);
                if (err_code == NRF_SUCCESS)
                {
                    ...
                    UNUSED_VARIABLE(nrf51_sdk_mem_free(p_alloc_buffer->buffer.p_memory));
...
uint32_t iot_pbuffer_free(iot_pbuffer_t  * p_pbuffer, bool free_flag)
{
    ...
    if (err_code == NRF_SUCCESS)
    {
        if (free_flag == true)
        {
            err_code = nrf51_sdk_mem_free(p_alloc_buffer->buffer.p_memory);
        }

 ...
 *          can be disabled by defining the ICMP6_DISABLE_LOGS.
 * @note That if ENABLE_DEBUG_LOG_SUPPORT is disabled, having ICMP6_DISABLE_LOGS has no effect.
 * @{
 */

#if (ICMP6_DISABLE_LOGS == 0)
 ...
 * @return      None. 
 */
static void app_notify_icmp_data(iot_interface_t  * p_interface, 
                                 ...
                                 uint32_t           process_result)
{
    if(m_event_handler != NULL)
    {
       ...
       icmp6_header_t * p_icmp_header = (icmp6_header_t *)
        ...
        UNUSED_VARIABLE(m_event_handler(p_interface,
                                        p_ip_header,
                                        p_icmp_header,
                                        process_result,
                                        p_pbuffer));
                     ...
                     iot_pbuffer_t    * p_packet)
{
    ...
    app_notify_icmp_data(p_interface, p_packet, process_result);

...
#define COAP_MESSAGE_ACK_SET(REMOTE, MID) { \
...
#define COAP_MESSAGE_RST_SET(REMOTE, MID) { \
    memcpy(&m_coap_empty_message.remote, (REMOTE), sizeof(coap_remote_t)); \
    m_coap_empty_message.header.id = (MID); \
...
static coap_message_t m_coap_empty_message = {
    ...
    .port.port_number  = COAP_SERVER_PORT,
    .options_len       = 0,
    .options_offset    = 0,
    .p_data            = NULL,
    .data_len          = 0  
                             ...
                             uint16_t               datalen)
{
    ...
    {
        COAP_MESSAGE_RST_SET(&p_message->remote, p_message->header.id);
    ...
    else if (is_response(p_message->header.code))
    {
        ...
        if (p_message->header.type == COAP_TYPE_CON)
        {
            ...
            COAP_MESSAGE_ACK_SET(&p_message->remote, p_message->header.id);
    ...
    else if (is_request(p_message->header.code))
    {
        ...
        if (found_resource != NULL)
        {
            if (found_resource->callback != NULL)
            {
                ...
                {
                    if ((coap_message_opt_present(p_message, COAP_OPT_OBSERVE) == NRF_SUCCESS) &&
                        (p_message->header.code == COAP_CODE_GET))
                    {
                        
                        coap_content_type_t ct_to_use;
                        coap_message_ct_match_select(&ct_to_use, p_message, found_resource);
                        // TODO: error check
                        
                        // Register observer, if successfull, add the Observe option in the reply.
                        uint32_t handle;
                        coap_observer_t observer;
                        
                        // Token Length.
                        observer.token_len            = p_message->header.token_len;
                        // Resource of interest.
                        observer.p_resource_of_interest = found_resource;
                        // Remote.
                        memcpy(&observer.remote, &p_message->remote, sizeof(coap_remote_t));
                        // Token.
                        memcpy(observer.token, p_message->token, observer.token_len);
                       
                        observer.ct = ct_to_use;
                        
                        err_code = internal_coap_observe_server_register(&handle, &observer);
                        
                        if (err_code == NRF_SUCCESS)
                        {
                            err_code = coap_message_opt_uint_add(p_response, COAP_OPT_OBSERVE, m_observe_sequence_number++);
                            
                            err_code = coap_message_opt_uint_add(p_response, COAP_OPT_MAX_AGE, found_resource->expire_time);
                        }
                    ...
                    found_resource->callback(found_resource, p_message, p_response);                    
        ...
        uint32_t handle;
        err_code = internal_coap_message_send(&handle, p_response);
        ...
        if (found_resource != NULL)
        {
            if (found_resource->callback != NULL)
            {
                ...
                {
                    // create a response message.
                    err_code = create_response(&p_error_response, p_message, COAP_MESSAGE_DATA_MAX_SIZE);
                    if (err_code != NRF_SUCCESS)
                    {
                        app_error_notify(err_code, p_message);
                    ...
                    p_error_response->header.code = COAP_CODE_405_METHOD_NOT_ALLOWED;
                }
            }
            else
            {
        ...
        else
        {
            ...
            p_error_response->header.code = COAP_CODE_404_NOT_FOUND;
        }
        ...
        if (p_error_response != NULL)
        {
            ...
            UNUSED_VARIABLE(nrf51_sdk_mem_free(p_error_response->p_data));

            COAP_TRC("[COAP]: Free mem, p_response = %p\r\n", (uint8_t *)p_error_response);
            UNUSED_VARIABLE(nrf51_sdk_mem_free((uint8_t *)p_error_response));
        }
...
uint32_t coap_message_new(coap_message_t ** p_request, coap_message_conf_t * p_config)
{
    ...
    if (is_request(p_config->code))
    {
        p_config->port.port_number = COAP_CLIENT_PORT;
    }
    else // RESPONSE
    {
        p_config->port.port_number = COAP_SERVER_PORT;
    }

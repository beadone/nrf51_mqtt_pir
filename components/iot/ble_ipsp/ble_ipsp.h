 ...
 *          over GATT. To exchange data, LE L2CAP Credit Mode is used. The PSM used for the channel is
 *          <TBD=0x1005(for now)> and is defined by the specification. The MTU mandated by the specification
 *          is 1280 bytes.
 *
 * @note Attention!
 ...
 *          This configuration also has implications
 *          on RAM usage, in combination with @ref BLE_IPSP_RX_BUFFER_SIZE. Therefore, make sure to tune these configuration options based
 *          on your application requirements and the permissible RAM usage for the module.
 */
 ...
 *
 * @returns    Provision for the application to indicate if the event was successfully processed or not. Currently not used.
 */
 ...
 *
 * @retval  NRF_SUCCESS  If initialization of the service was successful. Otherwise, an error code is returned.
 */
 ...
 * @param[in]   p_data    Pointer to memory containing the data to be transmitted. This memory must be resident and should therefore not be freed unless indicated
 *                        by the @ref BLE_IPSP_EVT_CHANNEL_DATA_TX_COMPLETE event.
 * @param[in]   data_len  Length/size of data to be transferred.
 *
 * @retval      NRF_SUCCESS If sending data was successful. Otherwise, an error code is returned.
 */
 ...
 *
 * @retval      NRF_SUCCESS If disconnecting was successful. Otherwise, an error code is returned.
 */
 ...
 *
 * @retval      NRF_SUCCESS If handling the event was successful. Otherwise, an error code is returned.
 */

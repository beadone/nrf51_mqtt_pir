...
//uncomment to get trace info
#define MQTT_TRC                   NRF_LOG_PRINTF
...
static err_t tcp_connection_callback(void * p_arg, struct tcp_pcb * p_pcb, err_t err)
{
    ...
    MQTT_TRC("[MQTT]: tcp_connection_callback\r\n");
    if (err == ERR_OK)
    {
        //Register callback.
...
uint32_t mqtt_connect(mqtt_client_t * p_client, const mqtt_connect_t * p_param)
{
    ...
    if (p_client == NULL)
    {
        ...
        //MQTT_TRC("[MQTT]: p_client == NULL err_code = MQTT_NULL_PARAM \r\n");
    }
    ...
    for (index = 0; index < MQTT_MAX_CLIENTS; index++)
    {
        ...
        {
        	//MQTT_TRC("[MQTT]: Found a free instance, allocate for new connection \r\n");
    ...
    if (index < MQTT_MAX_CLIENTS)
    {
        ...
        //MQTT_TRC("[MQTT]: mqtt_connect tcp_request_connection  \r\n");
        ...
        if(err_code != ERR_OK)
        {
            ...
            MQTT_TRC("[MQTT]: Free instance.  \r\n");
        }
        else
        {
            p_client = &index;
            //MQTT_TRC("[MQTT]: p_client = &index  \r\n");
        }
    }
    else
    {
    	MQTT_TRC("[MQTT]: problem here mqtt_connect index < MQTT_MAX_CLIENTS  \r\n");
    }
